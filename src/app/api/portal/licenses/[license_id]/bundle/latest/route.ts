...
import { getLatestPortalLicenseBundle } from "@/backend/portal/bundles"
...
export async function GET(
  ...
  try {
    ...
    const result = await getLatestPortalLicenseBundle(session.user, licenseId)

    if (!result.ok) {
      return jsonResponse({ message: result.message }, result.status)
    }

    return jsonResponse({ bundle: result.bundle })
  } catch {
